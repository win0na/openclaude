...
- OpenCode can talk to `openclaude` over a standard provider configuration
...
- prepares bootstrap config entries for the `openclaude` provider
...
`openclaude` now has two separate responsibilities.
...
7. OpenCode sends provider traffic to `openclaude serve`
...
`openclaude` can start even when the local `claude` CLI is missing, unauthenticated, or offline. In that case it falls back to the default Claude model catalog and logs a warning instead of silently assuming live Claude access is working.
